...
from carla.client import VehicleControl, make_carla_client
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        waypoints_filepath = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), WAYPOINTS_FILENAME
        )
        waypoints_np = None
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
                ...
                if local_waypoints != None:
                    ...
                    for i in range(1, local_waypoints_np.shape[0]):
                        wp_distance.append(
                            np.sqrt(
                                (
                                    local_waypoints_np[i, 0]
                                    - local_waypoints_np[i - 1, 0]
                                )
                                ** 2
                                + (
                                    local_waypoints_np[i, 1]
                                    - local_waypoints_np[i - 1, 1]
                                )
                                ** 2
                            )
                        )
                    ...
                    for i in range(local_waypoints_np.shape[0] - 1):
                        ...
                        for j in range(num_pts_to_interp):
                            next_wp_vector = (
                                INTERP_DISTANCE_RES * float(j + 1) * wp_uvector
                            )
                            wp_interp.append(
            ...
            if skip_first_frame and frame == 0:
                ...
                if lead_car_pos:  # If there exists a lead car, plot it
                    trajectory_fig.roll(
                        "leadcar", lead_car_pos[1][0], lead_car_pos[1][1]
                    )
                ...
                path_indices = np.floor(
                    np.linspace(0, wp_interp_np.shape[0] - 1, INTERP_MAX_POINTS_PLOT)
                )
                trajectory_fig.update(
                    "selected_path",
        ...
        write_trajectory_file(
            x_history, y_history, speed_history, time_history, collided_flag_history
        )
